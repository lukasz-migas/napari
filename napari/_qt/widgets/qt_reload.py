...
from napari._vendor.pydevd_reload.pydevd_reload import xreload
...
def install_hot_reload(window):
    """Install hot-reload module - recommended for developers only."""
    global _reload_ref

    run_reload = os.environ.get('NAPARI_HOT_RELOAD', '0') == '1'
    if run_reload and _reload_ref is None:
        modules = os.environ.get("NAPARI_HOT_RELOAD_MODULES", "")
        modules = modules.split(",") if modules else []
        modules = ["napari"] + modules
        _reload_ref = QtReloadWidget(list(set(modules)))
        if hasattr(window, "_update_theme_no_event"):
            _reload_ref.evt_theme.connect(window._update_theme_no_event)
